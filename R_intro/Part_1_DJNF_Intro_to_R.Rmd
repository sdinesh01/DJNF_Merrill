...
# returns column names, number of rows and columns, type (all character format)
str(MediaBucks)
```
...
#apparently head in R displays six rows by default??
head(MediaBucks, 6)
```
...
```{r}
#na.rm parameter removes na values -- returns nothing if you try to sum NA values
...
```{r}
# %>% operator joins lines (so you can write code on multiple lines, ctrl+shift+m)
